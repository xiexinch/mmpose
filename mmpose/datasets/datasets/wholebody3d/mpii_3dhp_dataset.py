...
class MPII3DHPWBDataset(BaseMocapDataset):
    ...
    def _load_annotations(self) -> Tuple[List[dict], List[dict]]:
        ...
        indices = list(range(len(self.ann_data)))
        subset_size = int(len(indices) * self.subset_frac)
        start = np.random.randint(0, len(indices) - subset_size + 1)
